  ...
  Game.TakePlaceInGame = function () {
      ...
      for (i = 0; i < Game.objects.length; ++i) {
		  ...
		  if (i === Game.objects.length - 1) {
		       ...
		       //traceVar(Game.objects[i].synch.serverTime); comment
		       //traceVar(gameStartTime); comment
		  }
 ...
 /*
  onCaptureLost = function(){
      gameStartTime === 0;   comment
      traceStr("GRISHUL"); comment
      //traceVar('NUMBER: ' + i); comment

  }

  onSynchReceived = function () {
      traceStr("Something changed");     comment
  }  */
